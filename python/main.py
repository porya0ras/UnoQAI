...
def execute_client_tool(tool_name, arguments):
    try:
        ...
        if tool_name == "write_led_matrix_text":
            ...
            print(f"LED matrix text: {arguments.get("text", "")}")
            return f"Wrote '{arguments.get("text", "")}' on my LED matrix.", "success"
...
def resolve_client_tool_requests(target_agent_id, response, client_tools):
    for _ in range(3):
        ...
        for message in getattr(response, "messages", []):
            ...
            result, status = execute_client_tool(tool_name, tool_arguments)
